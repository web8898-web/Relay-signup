...
export default function TaskListCard({ task, signups = [], accessToken, onEdit, onDelete, onShare }) {
  ...
  function handleTouchMove(e) {
    ...
    e.preventDefault();
    setDragging(true);
    setDragX(Math.min(0, Math.max(-st.cardWidth * 0.9, st.baseX + deltaX)));
  }
  ...
  function handleTouchEnd() {
    ...
    if (dragX <= -st.cardWidth * DELETE_FRACTION) {
      setRemoving(true);
  ...
  return (
    <div className="relative">
      ...
      >
        {dragX !== 0 && <div className="absolute inset-0 z-10 rounded-2xl" onClick={closeSwipe} aria-hidden="true" />}
        ...
        <div className="w-full px-4 py-3 flex items-center gap-1.5">
          ...
          <button
            ...
            className={`w-7 h-7 flex items-center justify-center shrink-0 transition ${notifyEnabled ? "text-emerald-500" : "text-gray-300"}`}
          ...
          <div className="relative shrink-0">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-9 bg-white border border-gray-100 rounded-xl shadow-lg py-1 z-20 w-44">
                {!confirmDelete ? (
                  <button onClick={() => setConfirmDelete(true)} className="w-full text-center px-3 py-2.5 text-sm text-rose-500 hover:bg-rose-50 rounded-lg">
                    移除
                  </button>
                ) : (
                  <div className="px-3 py-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setConfirmDelete(false)} className="flex-1 text-sm text-gray-500 py-2 rounded-lg hover:bg-gray-50">
                        否
                      </button>
                    </div>
                  </div>
        ...
        <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${expanded ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
          <div className="overflow-hidden">
            <div className="px-4 pb-4">
                ...
                {task.categories?.length > 0 && (
                  <div className="relative -mx-0.5">
                    <div ref={filterScrollRef} className="flex gap-1.5 overflow-x-auto pb-2 px-0.5">
                      <button onClick={() => setFilter("全部")} className={`shrink-0 text-[11px] px-2.5 py-1 rounded-full border ${filter === "全部" ? "bg-emerald-700 text-white border-emerald-700" : "bg-gray-50 text-gray-500 border-gray-200"}`}>
                        全部 <span className={`ml-1 ${filter === "全部" ? "text-white/70" : "text-gray-400"}`}>{signupCount}</span>
                      </button>
                      <button onClick={() => setFilter(NO_CATEGORY)} className={`shrink-0 text-[11px] px-2.5 py-1 rounded-full border ${filter === NO_CATEGORY ? "bg-emerald-700 text-white border-emerald-700" : "bg-white text-emerald-600/70 border-emerald-200 border-dashed"}`}>
                        沒選類別 <span className={`ml-1 ${filter === NO_CATEGORY ? "text-white/70" : "text-emerald-400"}`}>{noCategoryCount}</span>
                      </button>
                      {task.categories.map((c) => (
                        <button key={c} onClick={() => setFilter(c)} className={`shrink-0 text-[11px] px-2.5 py-1 rounded-full border ${filter === c ? "bg-emerald-700 text-white border-emerald-700" : "bg-gray-50 text-gray-500 border-gray-200"}`}>
                    ...
                    {filteredSignups.map((s, i) => (
                      <div
                        ...
                        className={`relative overflow-hidden flex items-start gap-2 border rounded-xl px-3 py-2 transition ${
                          ...
                          checkinMode
                            ? `cursor-pointer select-none active:scale-[0.99] ${checkedIds.has(s.id) ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-100"}`
              ...
              <div className="flex gap-2">
                <button onClick={onShare} className="flex-1 border border-emerald-200 text-emerald-600 rounded-full py-2.5 font-semibold flex items-center justify-center gap-2 hover:bg-emerald-50 transition">
                  <Share2 size={15} /> 分享
                </button>
                <button onClick={onEdit} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full py-2.5 font-semibold flex items-center justify-center gap-2 transition">
                  <Edit2 size={15} /> 編輯任務
                </button>
              </div>
            </div>
