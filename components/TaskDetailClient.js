...
import { Send, Users, CheckCircle2, ChevronRight, AlertCircle, Pencil, XCircle } from "lucide-react";
...
export default function TaskDetailClient() {
  ...
  useEffect(() => {
    if (!myWaitingSignup) {
      ...
      setShowCancelQueueConfirm(false);
    }
  }, [myWaitingSignup?.id]);
  ...
  async function handleSend() {
    if (!task) return;
    if (isQueueTask && myWaitingSignup) {
    ...
    try {
      ...
      if (!isQueueTask) startCooldown(30);
      await refreshSignups();
      requestAnimationFrame(() => {
  ...
  async function handleDelete(signupId, successMessage = "已刪除") {
  ...
  function beginQueueNameEdit() {
    if (!myWaitingSignup) return;
    setQueueNameDraft(myWaitingSignup.name || "");
    setEditingQueueName(true);
  }

  async function saveQueueName() {
    const nextName = queueNameDraft.trim();
    if (!myWaitingSignup || !nextName) {
      showToast("請輸入姓名");
      return;
    }
    setQueueActionBusy(true);
    const ok = await handleUpdate(myWaitingSignup.id, { name: nextName });
  ...
  async function confirmCancelQueue() {
    if (!myWaitingSignup) return;
    setQueueActionBusy(true);
    const ok = await handleDelete(myWaitingSignup.id, "已取消排隊");
    setQueueActionBusy(false);
    if (ok) {
      setShowCancelQueueConfirm(false);
      setEditingQueueName(false);
  ...
  return (
    <>
      <FadeIn className="flex-1 flex flex-col relative min-w-0">
              ...
              {myAllCompleted ? (
                ...
                <div className="px-5 py-5 text-center bg-gradient-to-br from-sky-50 via-white to-emerald-50">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-2xl bg-white/80 border border-emerald-100 px-2 py-2 flex flex-col items-center justify-center">
                      <span className="relative flex h-2.5 w-2.5 mb-1.5" aria-hidden="true">
                        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
                        <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500" />
                      </span>
                      <p className="text-[12px] leading-tight font-semibold text-emerald-700 whitespace-nowrap">排隊狀態即時同步</p>
                    </div>
                  </div>

                  {editingQueueName ? (
                    <div className="mt-4 rounded-2xl border border-sky-100 bg-white/90 p-3 text-left">
                      <label className="block text-xs font-semibold text-gray-500 mb-1.5">更改名字</label>
                      <input
                        value={queueNameDraft}
                        onChange={(e) => setQueueNameDraft(e.target.value)}
                        maxLength={60}
                        autoFocus
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          type="button"
                          onClick={() => setEditingQueueName(false)}
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2 border-t border-sky-100 pt-4">
                      <button
                        type="button"
                        onClick={beginQueueNameEdit}
                        className="rounded-2xl border border-sky-200 bg-white py-2.5 text-sm font-semibold text-sky-700 flex items-center justify-center gap-1.5"
                      >
                        <Pencil size={15} /> 更改名字
                      </button>
                      <button
                        type="button"
                        onClick={() => setShowCancelQueueConfirm(true)}
                        className="rounded-2xl border border-rose-200 bg-white py-2.5 text-sm font-semibold text-rose-500 flex items-center justify-center gap-1.5"
                      >
                        <XCircle size={16} /> 不排了
                      </button>
                    </div>
        ...
        ) : isQueueTask && myWaitingSignup ? null : full ? (
          <div ref={formSectionRef} className="px-6 pb-6 pt-2 text-center text-xs text-gray-400 border-t border-gray-100">
            這個任務已經額滿，無法再接龍
          </div>
        ) : viewOnly ? (
      ...
      {showCancelQueueConfirm && myWaitingSignup && (
        <div className="fixed inset-0 z-[12000] flex items-center justify-center bg-black/40 px-6" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm rounded-3xl bg-white p-5 shadow-2xl">
            ...
            <p className="mt-2 text-center text-sm leading-relaxed text-gray-500">
              取消後會失去目前順位，若重新加入，將排到最後一位。
            </p>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setShowCancelQueueConfirm(false)}
                disabled={queueActionBusy}
