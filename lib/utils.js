...
const NATIVE_SHARE_ENABLED_MARKER = "__relay_native_share_enabled__";
...
export function getVisibleTaskNote(value = "") {
  return String(value)
    .split(/\r?\n/)
    .filter((line) => !CONFIG_MARKERS.has(line.trim()) && !line.trim().startsWith(BANNER_PREFIX))
    .join("\n")
    .trim();
}

export function getTaskBannerUrl(task) {
  const line = String(task?.note || "")
    .split(/\r?\n/)
    .map((item) => item.trim())
    .find((item) => item.startsWith(BANNER_PREFIX));
  if (!line) return "";
  const url = line.slice(BANNER_PREFIX.length).trim();
...
export function shouldShowShareButton(task) {
  ...
  if (noteLines.includes(NATIVE_SHARE_ENABLED_MARKER)) return true;
...
export function formatTaskDateRange(startValue, endValue) {
...
export function buildShareText(task, url) {
  ...
  const lines = [
    ...
    `🗓 ${formatTaskDateRange(task.start_date, task.end_date)}`,
...
export function buildFlexMessage(task, url) {
  ...
  bodyContents.push({
    type: "text",
    text: `🗓 ${formatTaskDateRange(task.start_date, task.end_date)}`,
    size: "sm",
    color: "#9ca3af",
